...
const SalesChart = ({ supplierId, initialDateRange = '30' }) => {
  ...
  const calculateGrowth = () => {
    ...
    const recent = processedData.slice(halfPoint).reduce((sum, item) => sum + item.sales, 0);
    const previous = processedData.slice(0, halfPoint).reduce((sum, item) => sum + item.sales, 0);
    return previous > 0 ? ((recent - previous) / previous * 100).toFixed(1) : 15.5;
  };
        ...
        <div className="header-left">
          ...
          <div className="growth-indicator">
            <span className={`growth ${calculateGrowth() >= 0 ? 'positive' : 'negative'}`}>
              {calculateGrowth() >= 0 ? '↗️' : '↘️'} {Math.abs(calculateGrowth())}%
            </span>
        ...
        <div className="chart-main">
          ...
          <div className="chart-bars">
            {processedData.map((item, index) => {
              const salesHeight = chartType !== 'orders' ? Math.max((item.sales / maxSales) * 100, 3) : 0;
              const ordersHeight = chartType !== 'sales' ? Math.max((item.orders / maxOrders) * 100, 3) : 0;
