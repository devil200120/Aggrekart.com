...
const SupplierOrdersPage = () => {
  ...
  const getItemPrice = (item) => {
    return item.totalPrice || (item.unitPrice * item.quantity) || (item.price * item.quantity) || 0
  }
        ...
        <div className="stats-grid">
          ...
          <div className="stat-card processing">
            <div className="stat-content">
              ...
              <div className="stat-value">{(stats.confirmed || 0) + (stats.preparing || 0) + (stats.material_loading || 0) + (stats.processing || 0)}</div>
            </div>
          </div>
                  ...
                  <div key={order._id} className="order-card">
                    ...
                    <div className="order-content">
                      ...
                      <div className="items-section">
                        <h4><Package2 size={16} /> Order Items ({order.items?.length || 0})</h4>
                        <div className="items-list">
                          {order.items?.map((item, index) => (
                            <div key={index} className="order-item">
                              <div className="item-details">
                                <span className="item-name">{item.productSnapshot?.name || item.product?.name || 'Unknown Product'}</span>
                                <span className="item-quantity">Qty: {item.quantity} {item.productSnapshot?.unit || ''}</span>
                                <span className="item-unit-price">Unit Price: {formatCurrency(item.unitPrice || item.price || 0)}</span>
                              </div>
                              <div className="item-total-section">
                                <span className="item-price">{formatCurrency(getItemPrice(item))}</span>
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      {order.pricing && (
                        <div className="pricing-section">
                          <h4><IndianRupee size={16} /> Pricing Details</h4>
                          <div className="pricing-breakdown">
                            ...
                            {order.pricing.transportCost > 0 && (
                              <div className="pricing-row">
                                ...
                                <span>{formatCurrency(order.pricing.transportCost)}</span>
                              </div>
                            )}
                            ...
                            <div className="payment-detail">
                              ...
                              <span className={`payment-status ${order.payment.status}`}>
                                {order.payment.status === 'paid' ? '✅ Paid' :
                                 ...
                                 order.payment.method === 'cod' ? '💰 Pay on Delivery' : '⏳ Pending'}
                              </span>
                            </div>
                            {order.payment.advanceAmount && (
                              <div className="payment-detail">
                                <span className="label">Advance Paid:</span>
                                <span>{formatCurrency(order.payment.advanceAmount)}</span>
                              </div>
                            )}
                        ...
                        <div className="address-info">
                          <div className="address-text">
                            ...
                            {!deliveryAddress.address && !deliveryAddress.city && (
                              <div className="no-address">Address information not available</div>
                            )}
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="order-footer">
                      <div className="order-total">
                        ...
                        <span className="total-amount">{formatCurrency(orderTotal)}</span>
                      </div>
                      ...
                      <div className="order-actions">
                        ...
                        {nextStatus && statusInfo.action && (
                          ...
                          >
                            {updateStatusMutation.isLoading ? 'Updating...' : statusInfo.action}
                          </button>
                        )}
                      </div>
                    </div>
            ...
            <div className="modal-body">
              ...
              <div className="order-detail-section">
                ...
                <div className="detail-grid">
                  ...
                  <div><strong>Phone:</strong> {selectedOrder.customer?.phoneNumber || 'N/A'}</div>
              ...
              {/* Order Items Details */}
              <div className="order-detail-section">
                <h3><Package2 size={18} /> Order Items</h3>
                <div className="detailed-items">
                  {selectedOrder.items?.map((item, index) => (
                    <div key={index} className="detailed-item">
                      <div className="item-info">
                        <strong>{item.productSnapshot?.name || item.product?.name || 'Unknown Product'}</strong>
                        <div className="item-specs">
                          <span>Quantity: {item.quantity} {item.productSnapshot?.unit || ''}</span>
                          <span>Unit Price: {formatCurrency(item.unitPrice || item.price || 0)}</span>
                          <span>Total: {formatCurrency(getItemPrice(item))}</span>
                        </div>
                        {item.specifications?.customRequirements && (
                          <div className="custom-requirements">
                            <strong>Custom Requirements:</strong> {item.specifications.customRequirements}
                          </div>
                        )}
                      </div>
                    </div>
              ...
              {selectedOrder.pricing && (
                <div className="order-detail-section">
                  <h3><IndianRupee size={18} /> Pricing Breakdown</h3>
                  <div className="pricing-details">
                    ...
                    {selectedOrder.pricing.transportCost > 0 && (
                      <div className="pricing-row">
                        <span>Transport Cost:</span>
                        <span>{formatCurrency(selectedOrder.pricing.transportCost)}</span>
                      </div>
                    )}
                    {selectedOrder.pricing.gstAmount > 0 && (
                      <div className="pricing-row">
                        <span>GST Amount:</span>
                        <span>{formatCurrency(selectedOrder.pricing.gstAmount)}</span>
                      </div>
                    )}
                    {selectedOrder.pricing.commission > 0 && (
                      <div className="pricing-row">
                        <span>Platform Commission:</span>
                        <span>{formatCurrency(selectedOrder.pricing.commission)}</span>
                      </div>
                    )}
                    <div className="pricing-row total">
              ...
              {selectedOrder.payment && (
                <div className="order-detail-section">
                  ...
                  <div className="detail-grid">
                    <div><strong>Method:</strong> {selectedOrder.payment.method?.toUpperCase() || 'N/A'}</div>
                    <div><strong>Status:</strong> {selectedOrder.payment.status?.toUpperCase() || 'N/A'}</div>
                    {selectedOrder.payment.advancePercentage && (
                      <div><strong>Advance %:</strong> {selectedOrder.payment.advancePercentage}%</div>
                    )}
              ...
              {selectedOrder.timeline && selectedOrder.timeline.length > 0 && (
                <div className="order-detail-section">
                  <h3><Calendar size={18} /> Order Timeline</h3>
                  <div className="timeline">
                    {selectedOrder.timeline.map((timeline, index) => (
                      <div key={index} className="timeline-item">
                        <div className="timeline-status">{timeline.status}</div>
                        <div className="timeline-date">{formatDate(timeline.timestamp)}</div>
                        {timeline.note && <div className="timeline-note">{timeline.note}</div>}
                      </div>
                    ))}
                  </div>
                </div>
